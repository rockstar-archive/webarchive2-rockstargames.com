...
{
	//RM - Bypass Flash check
	this.foundVersion = 30;
	...
	if (navigator.userAgent.toLowerCase().indexOf("opera") < 0) _isIE4 = (document.all) ? true : false;
	...
	function _detectJS()
	{
		...
		if (navigator.plugins)
		{
			...
			for (var i = 0; i < navigator.plugins.length; i++)
			{
				...
				{
					var stringVersion = navigator.plugins[i].description.substring(navigator.plugins[i].description.indexOf(".") - 2, navigator.plugins[i].description.length);
					...
					var currentVersion = (stringMajorVersion + "." + ((stringMinorVersion < 1000) ? "0" : "") + ((stringMinorVersion < 100) ? "0" : "") + ((stringMinorVersion < 10) ? "0" : "") + stringMinorVersion);
					if (currentVersion > this.foundVersion) this.foundVersion = currentVersion;
					if (DEBUG_MODE) alert('DEBUG: Found Version - ' + currentVersion);
				}
			}
		}
		...
		if ((navigator.mimeTypes) && (!this.foundVersion > 0))
		{
			...
			if ((navigator.mimeTypes["application/x-shockwave-flash"]) && (navigator.mimeTypes["application/x-shockwave-flash"].enabledPlugin))
			{
				...
				{
					var stringVersion = mimeObject.description.substring(mimeObject.description.indexOf(".") - 2, mimeObject.description.length);
					var stringMajorVersion = parseInt(stringVersion, 10);
					var stringMinorVersion = parseInt(stringVersion.substring(stringVersion.indexOf("r") + 1, stringVersion.length), 10);
					var currentVersion = (stringMajorVersion + "." + ((stringMinorVersion < 1000) ? "0" : "") + ((stringMinorVersion < 100) ? "0" : "") + ((stringMinorVersion < 10) ? "0" : "") + stringMinorVersion);
					if (currentVersion > this.foundVersion) this.foundVersion = currentVersion;
