function theDudeAbides(){
	document.write('<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=8,0,0,0" width="100%" height="100%" id="splash" align="middle">');
	document.write('<param name="allowScriptAccess" value="sameDomain" />');
	document.write('<param name="movie" value="splash.swf" />');
	document.write('<param name="menu" value="false" /><param name="quality" value="high" />');
	document.write('<param name="scale" value="noscale" /><param name="bgcolor" value="#000000" />');
	document.write('<embed src="splash.swf" menu="false" quality="high" scale="noscale" bgcolor="#000000" width="100%" height="100%" name="splash" align="middle" allowScriptAccess="sameDomain" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />');
	document.write('</object>');
}